...
void write_code_chunk(tll_code_chunk *code_chunk, uint8_t byte, int line)
{
    if (code_chunk->count + 1 > code_chunk->capacity)
    {
        ...
        code_chunk->lines = GROW_ARRAY(int, code_chunk->lines, old_capacity, code_chunk->capacity);
        code_chunk->code = GROW_ARRAY(uint8_t, code_chunk->code, old_capacity, code_chunk->capacity);
    }
    code_chunk->lines[code_chunk->count] = line;
